...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def sam_buttons_colors(self, mode):
        ...
        [add_style, add_hover] = [
            green, green] if mode == "add" else [trans, hoverColor]
        [remove_style, remove_hover] = [
            red, red] if mode == "remove" else [trans, hoverColor]
        [rect_style, rect_hover] = [
            green, green] if mode == "rect" else [trans, hoverColor]
        [clear_style, clear_hover] = [
            red, red] if mode == "clear" else [trans, hoverColor]
        [finish_style, finish_hover] = [
            black, black] if mode == "finish" else [trans, hoverColor]

        self.sam_add_point_button.setStyleSheet(
            style_sheet_const + add_style + ";}" + hover_const + add_hover + ";}")
        self.sam_remove_point_button.setStyleSheet(
            style_sheet_const + remove_style + ";}" + hover_const + remove_hover + ";}")
        self.sam_select_rect_button.setStyleSheet(
            style_sheet_const + rect_style + ";}" + hover_const + rect_hover + ";}")
        self.sam_clear_annotation_button.setStyleSheet(
            style_sheet_const + clear_style + ";}" + hover_const + clear_hover + ";}")
        self.sam_finish_annotation_button.setStyleSheet(
            style_sheet_const + finish_style + ";}" + hover_const + finish_hover + ";}")
